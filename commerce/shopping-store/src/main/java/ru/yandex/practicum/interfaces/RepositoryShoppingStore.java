...
public interface RepositoryShoppingStore extends JpaRepository<ProductOfStore, UUID> {

    @Query("SELECT p FROM Product p WHERE p.productCategory = :category AND p.productState = 'ACTIVE'")
    Page<ProductOfStore> findAllByProductCategory(@Param("category") ProductCategory category, Pageable pageable);
